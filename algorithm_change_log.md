...
- Tested a larger xstrong INR configuration. It changed only one top105 boundary swap versus the strong INR version, replacing `000173.jpg` with `000074.jpg`, and the public score decreased. This supports the Fit-a-NeF-style lesson that larger/better-fitting INR settings do not necessarily improve downstream classification; within the INR branch, the strong INR configuration remained better than xstrong.
...
- Submitted DINOv2 LoRA quick `top107`; score was `0.79792`, below `top106`, so the extra rank-107 image is likely negative under the current LoRA ranking.
- Tested blending the current LoRA route with INR; `top106` scored `0.79166`, below pure LoRA `top106`. Current evidence: INR helped the frozen-feature pipeline but should not be blended into the best LoRA submission.
- Tested LoRA quick `seed4` at `top105`; score dropped to `0.74345`. This seed changed 26 labels versus the best quick `top105`, so future LoRA seed ensembling should filter bad seeds before averaging.
- Reorganized the repository documentation so `readme.md`, `key_technical_explanation_cn.md`, and `technical_report_current_best.md` match the current code structure: root-level LoRA pipeline as the final route, historical attempts under `historical_attempts/`.
- Promoted `train_dinov2_lora_v2.py` to the current best route after it reached `0.80628` with `python train_dinov2_lora_v2.py --data-root .. --batch-size 4 --amp`. The v2 route does not use INR; INR is now documented only under tried-but-unused techniques.
